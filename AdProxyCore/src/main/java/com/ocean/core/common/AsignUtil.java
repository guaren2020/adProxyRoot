                ...
                for(short j=0;j<fields_t.length;j++){
                	 ...
                	 if(field_TName.equals(field_FName)&&!Modifier.isStatic(field_f.getModifiers())){//如果有相同的geter和setter方法就将通过getter和setter方法进行赋值。
                         ...
                         try {
                             ...
                             throw new BusinessException(ErrorCode.INTER_ERROR,"SecurityException:object automatic assign fire exception!");
                         } catch (NoSuchMethodException e) {
                        	 throw new BusinessException(ErrorCode.INTER_ERROR,"NoSuchMethodException:object  automatic assign fire exception!");
                         } catch (IllegalArgumentException e) {
                        	 throw new BusinessException(ErrorCode.INTER_ERROR,"IllegalArgumentException:object  automatic assign fire exception!");
                         } catch (IllegalAccessException e) {
                        	 throw new BusinessException(ErrorCode.INTER_ERROR,"IllegalAccessException:object  automatic assign fire exception!");
                         } catch (InvocationTargetException e) {
                        	 throw new BusinessException(ErrorCode.INTER_ERROR,"InvocationTargetException:object  automatic assign fire exception!");
                         } finally {
                             //清理资源
                         }
                	 }
